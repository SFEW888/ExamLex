...
class WheelSmokeScriptTests(unittest.TestCase):
    ...
    def test_smoke_script_checks_commands_and_packaged_resources(self):
        ...
        self.assertIn('"strategy-db", "--help"', source)
        self.assertIn('strategy_database_round_trip', source)
        self.assertIn('strategy-library-export.json', source)
        self.assertIn('"capacity-monitor", "--help"', source)
        self.assertIn('"validate-exam-artifact", "--help"', source)
        self.assertIn('"vocab-card", "--help"', source)
        self.assertIn('"source-list", "--collectable", "--json"', source)
        self.assertIn("'assets/schemas': canonical_root / 'assets' / 'schemas'", source)
        self.assertIn("'assets/templates': canonical_root / 'assets' / 'templates'", source)
        self.assertIn("'references': canonical_root / 'references'", source)
        self.assertIn("'tutor-role-contracts': canonical_root / 'references'", source)
        self.assertIn("'tutor-runtime': canonical_root / 'references'", source)
        self.assertIn("'source-collection': canonical_root / 'references'", source)
        self.assertIn("'source-catalog': canonical_root / 'assets' / 'data'", source)
        self.assertIn("load_role_contracts", source)
        self.assertIn("prepare_tutor_turn", source)
        self.assertIn("load_source_catalog", source)
        self.assertIn("'SKILL.md': canonical_root / 'SKILL.md'", source)
        self.assertIn("assert not (compatibility_root / 'assets').exists()", source)
