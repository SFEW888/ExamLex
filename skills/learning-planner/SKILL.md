...
This is a shortcut Skill for the learning planning assistant in `english-exam-ai-tutor`.

Use the public-safe assistant boundary from `../english-exam-ai-tutor/references/assistant-roster.md`. If the full local prompt mode is explicitly configured, use the user's private local prompt asset without copying, rewriting, or publishing that prompt text.
...
4. Generate a constrained daily or staged plan with the main English Exam AI Tutor workflow.
5. Ground every adjustment in practice records, error summaries, and ability trends.

When the user has ingested strategies (via book, video, person, or direct text), always reference `strategy-library.json` during planning. See `../english-exam-ai-tutor/references/multi-source-distillation.md`.
