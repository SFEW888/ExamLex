...
> All five distillation paths are built into english-exam-ai-tutor. No external skills or tools needed (except yt-dlp + ffmpeg for video download). The Agent orchestrates a 5-stage pipeline: Extract → Distill → Validate → Evaluate → Commit.
...
All intermediate artifacts are stored under `~/.english-exam-ai-tutor/sessions/<date>/<uuid>/`.
Long-running distillations can be resumed:
```bash
tutor resume <session-id>
```
