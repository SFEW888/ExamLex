...
使用前运行 `python run.py prompt-check --private-dir <path> --save`。需要机器可读元数据时添加 `--json`。该命令校验文件名和文件安全性，只报告字节大小与 SHA-256 哈希，绝不返回提示词正文或已配置路径。有限澄清与受信进程内提供器边界见 [tutor-runtime.md](tutor-runtime.md)。
...
- 不得通过 stdout、Shell 参数、临时文件、工具结果或普通日志传递组合提示词；没有受信提供器时保持 public-safe。
